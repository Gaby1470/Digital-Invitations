...
async function submitRsvp(slug: string, submission: RsvpSubmission): Promise<any> {
    const res = await fetch(`/api/rsvps/${slug}`, {
        method: 'POST',
  ...
  const RsvpForm = () => {
    ...
    const rsvpMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            queryClient.setQueryData(['partyDetails', slug], (oldData: any) => {
                return {
                    ...oldData,
                    rsvp: data
                }
        ...
        <div className="p-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button type="submit" disabled={rsvpMutation.isLoading} className="w-full flex items-center justify-center px-6 py-4 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-indigo-400 transition-all text-lg shadow-md">
                        {rsvpMutation.isLoading ? <><Loader className="animate-spin mr-2"/> Enviando...</> : <><Send className="mr-2"/> {party?.rsvp ? 'Actualizar RSVP' : 'Enviar RSVP'}</>}
                    </button>
                </div>
            </form>
        </div>
